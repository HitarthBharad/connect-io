...
export default function EditThought() {
    ...
    const { updateThought, getThought, removeThought } = useThoughts();
    ...
    const editableThought = getThought();

    const [formData, setFormData] = useState({
        name: editableThought?.name || "",
        text: editableThought?.text || "",
        topics: editableThought?.topics || [],
    });

    if(!editableThought || editableThought == null) {
    ...
    const onSubmit = async () => {
        ...
        const thoght = await updateThought(editableThought._id, formData);
    ...
    return (
        <div className="grid gap-4">
            <div className="flex items-center justify-between">
                <h1 className="">Edit your entry</h1>
                <Button
                    color="primary"
                    onClick={onSubmit}
                    className="w-32 mt-4"
